...
for(const book of allBookings){
        let currentEnd = new Date(endDate).toJSON().slice(0,10);
        let currentStart = new Date(startDate).toJSON().slice(0,10);
        let from = new Date(book.startDate);//oldstart
        let to   = new Date(book.endDate);//oldend
        let check1 = new Date(currentStart);//new date
        let check2 = new Date(currentEnd)//new end
        
        if (check1 >= from && check1 <=to) {start = true}
        if (check2 >= from && check2 <=to) {end = true}
}

const isBetween = await Booking.findAll({where: {spotId: booking.spotId},where: {
    startDate: {[Op.between]: [startDate, endDate]},
    endDate: {[Op.between]: [startDate, endDate]},
    spotId: booking.spotId
}})

    if([...isBetween][0]){errors["conflict"] = "Dates in between conflict with an exisiting booking"}
    if(start === true) {errors["startDate"] = "Start date conflicts with an existing booking"}
    if(end === true) {errors["endDate"] = "End date conflicts with an existing booking"}
    if(start || end || ([...isBetween][0])) {return res.status(403).json({"message": "Sorry, this spot is already booked for the specified dates", errors})}
